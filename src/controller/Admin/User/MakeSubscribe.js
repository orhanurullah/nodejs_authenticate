...
const MakeSubscribe = async (req, res) => {
   if(!req.params?.id){
   ...
   await user.addSubscribe(section);
   const control = await user.save();
   if(control){
        return res.status(200).json({
            error:false,
            message: 'Kullanıcı için abonelik eklemesi gerçekleşti.'
        });
   }else{
    return res.status(404).json({
        error:true,
        message: 'Kullanıcı için abonelik ekleme işlemi tamamlanamadı. Bilinmeyen bir hata oluştu'
    });
   }
}
